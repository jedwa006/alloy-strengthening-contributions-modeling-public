...
CELLS: list[nbf.NotebookNode] = [
    ...
    md(
        "## 3. M54 calibration from your cw/cr austenite data",
        "",
        "Your measurements (cw_pct, austenite vol fraction) at 0/20/40/60% cumulative",
        "cold rolling are the right calibration target for M54 reverted-austenite",
        "Olson-Cohen parameters — much better than transferring 304 SS values.",
        "",
        "**Edit `src/m54model/calibration/user_trip_data.py`** to drop in your real",
        "values, then re-run this cell. The placeholder values below give a generic",
        "TRIP-steel response so the fitter runs end-to-end.",
    ),
    code(
        "from m54model.calibration.user_trip_data import (",
        "    USER_CW_AUSTENITE_DATA_PLACEHOLDER, m54_olson_cohen_fit_from_user_data,",
        "    cw_pct_to_true_strain,",
        ")",
        "",
        "data = USER_CW_AUSTENITE_DATA_PLACEHOLDER",
        "print('Input measurements:')",
        "for p in data:",
        "    eps_eq = cw_pct_to_true_strain(p.cw_pct)",
        "    print(f'  cw={p.cw_pct:5.1f}% (ε≈{eps_eq:.3f})  f_A = {p.f_austenite:.4f}')",
        "",
        "fitted = m54_olson_cohen_fit_from_user_data(data)",
        "print()",
        "print(f'Fitted M54 Olson-Cohen params at {fitted.T_celsius} °C:')",
        "print(f'  α = {fitted.alpha:.3f}')",
        "print(f'  β = {fitted.beta:.3f}')",
        "print(f'  n = {fitted.n:.2f} (held fixed)')",
    ),
    md(
        "### Plot the fit against your data",
        "",
        "If the fit looks bad, consider varying `n` (default 4.5 from Olson-Cohen 304 SS;",
        "less-twinning-prone alloys may want a different exponent).",
    ),
    code(
        "import math as _math",
        "baseline = data[0]",
        "eps_user = [cw_pct_to_true_strain(p.cw_pct) for p in data[1:]]",
        "fap_user = [(baseline.f_austenite - p.f_austenite) / baseline.f_austenite for p in data[1:]]",
        "",
        "eps_smooth = np.linspace(0, max(eps_user) * 1.1, 100)",
        "f_smooth = [olson_cohen_volume_fraction(e, fitted) for e in eps_smooth]",
        "",
        "fig, ax = plt.subplots(figsize=(8, 5))",
        "ax.plot(eps_smooth, f_smooth, '-', color='#1f78b4', label='Olson-Cohen fit')",
        "ax.scatter(eps_user, fap_user, color='red', s=80, zorder=5, label='Your cw/cr data')",
        "ax.set_xlabel('Plastic strain ε  (= -ln(1 - cw))')",
        "ax.set_ylabel('Strain-induced α′ fraction (of available austenite)')",
        "ax.set_title('M54 reverted-austenite Olson-Cohen calibration')",
        "ax.legend()",
        "ax.grid(alpha=0.4)",
        "fig.tight_layout()",
        "plt.show()",
